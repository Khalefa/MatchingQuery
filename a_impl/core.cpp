...
{
                producers.push_back(thread(MatchDocument_singlethread, doc_id, (doc_str) ));
//MatchDocument_singlethread( doc_id, doc_str);
		return EC_SUCCESS;
}
...
//	sleep(10);
       unique_lock<mutex> lock(xmutex);
      is_not_empty.wait(lock, [] { return docs.size() >0; });
